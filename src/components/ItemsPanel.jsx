...
export function ItemsPanel({ items, updateItems, showToast }) {
  ...
  const preview = useMemo(
    () =>
      csvText.trim() ? parseItemsCsv(csvText) : { parsed: [], skipped: 0 },
    [csvText],
  );
  ...
  function handleCsvFile(e) {
    ...
    reader.onerror = () =>
      setImportError(
        "Couldn't read that file — try again or paste the data instead.",
      );
  ...
  function commitImport() {
    if (preview.parsed.length === 0) {
      setImportError(
        "Nothing to import — check that your file has a Description column.",
      );
      return;
    }
    updateItems([...preview.parsed, ...items]);
    showToast(
      `Imported ${preview.parsed.length} item${preview.parsed.length === 1 ? "" : "s"}${preview.skipped ? ` · skipped ${preview.skipped}` : ""}`,
    );
    setCsvText("");
    setImportError("");
    setShowImport(false);
  ...
  function handleItemImage(e) {
    ...
    const reader = new FileReader();
    reader.onload = () =>
      setForm((f) => ({ ...f, imageDataUrl: reader.result }));
    reader.onerror = () =>
      setImageError("Couldn't read that file — try a different image.");
  ...
  function addOrUpdate() {
    ...
    if (editingId) {
      updateItems(
        items.map((it) => (it.id === editingId ? { ...it, ...form } : it)),
      );
  ...
  function edit(it) {
    setForm({
      code: it.code || "",
      desc: it.desc,
      category: it.category || ITEM_CATEGORIES[0],
      unit: it.unit || "m2",
        ...
        <div className="flex gap-2 flex-wrap">
          {items.length > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 bg-transparent border border-[#DAD5C6] text-[#1B2A3D] px-4 py-2 rounded-md text-[13.5px] font-semibold"
            >
        ...
        <div className="border border-[#E4DFD3] rounded-lg p-4.5 bg-[#FFFDF9] mb-5.5">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2.5 mb-3">
            <div>
              ...
              <div className="text-[12.5px] text-[#8A8574]">
                Upload a CSV with columns: Category, Code, Description, Supplier
                Name, Unit, Cost, Rate. Only Description is required.
              </div>
            </div>
            <button
              onClick={downloadTemplate}
              className="flex items-center gap-1.5 bg-transparent border border-[#DAD5C6] rounded-md px-3 py-1.5 text-[12.5px] font-semibold text-[#1B2A3D] shrink-0 self-start"
            >
          ...
          <div className="flex gap-4 mb-3 flex-wrap">
            ...
            {csvText && (
              <button
                ...
                className="bg-transparent border border-[#DAD5C6] rounded-md px-3.5 py-2 text-[13px]"
              >
                Clear
              </button>
            )}
          ...
          {csvText.trim() && (
            <div
              className={`text-[12.5px] mb-2.5 font-semibold ${preview.parsed.length ? "text-[#3D6B5C]" : "text-[#B5482F]"}`}
            >
              {preview.parsed.length} item
              {preview.parsed.length === 1 ? "" : "s"} ready to import
              {preview.skipped
                ? ` · ${preview.skipped} row${preview.skipped === 1 ? "" : "s"} skipped (missing description)`
                : ""}
            </div>
          )}
          ...
          {preview.parsed.length > 0 && (
            <div className="border border-[#E4DFD3] rounded-md overflow-hidden mb-3.5 max-h-[180px] overflow-y-auto">
                  ...
                  <thead>
                    <tr className="bg-[#F1EDE3] text-left sticky top-0">
                      {[
                        ...
                        <th
                          key={h}
                          className="px-2.5 py-1.5 text-[10px] font-mono text-[#8A8574] uppercase"
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {preview.parsed.slice(0, 8).map((it) => (
                      <tr key={it.id} className="border-t border-[#EDEAE2]">
                        ...
                        <td className="font-mono px-2.5 py-1.5">
                          {it.code || "—"}
                        </td>
              ...
              {preview.parsed.length > 8 && (
                <div className="px-2.5 py-1.5 text-[11.5px] text-[#8A8574]">
                  + {preview.parsed.length - 8} more…
                </div>
              )}
            </div>
          )}
          ...
          {importError && (
            <div className="text-[#B5482F] text-[12.5px] mb-2.5">
              {importError}
            </div>
          )}
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`border-none px-4 py-2 rounded-md text-[13.5px] font-semibold ${preview.parsed.length ? "bg-[#1B2A3D] text-[#FAF8F3] cursor-pointer" : "bg-[#DAD5C6] text-[#FAF8F3] cursor-not-allowed"}`}
            >
              Import {preview.parsed.length || ""} item
              {preview.parsed.length === 1 ? "" : "s"}
            </button>
            <button
              onClick={() => setShowImport(false)}
              className="bg-transparent border border-[#DAD5C6] px-4 py-2 rounded-md text-[13.5px]"
            >
              Cancel
            </button>
          </div>
          ...
          <div className="border border-[#E4DFD3] rounded-lg p-4.5 bg-[#FFFDF9]">
              ...
              <div className="flex items-center gap-3.5">
                <div className="w-14 h-14 rounded-lg border border-[#DAD5C6] bg-[#FFFDF9] flex items-center justify-center overflow-hidden shrink-0">
                  {form.imageDataUrl ? (
                    ...
                    <span className="font-mono text-[10px] text-[#8A8574]">
                      none
                    </span>
                  )}
                </div>
                <div>
                  ...
                  {form.imageDataUrl && (
                    <button
                      ...
                      className="ml-2 bg-transparent border-none text-[#B5482F] text-[12.5px]"
                    >
                      Remove
                    </button>
                  )}
                  {imageError && (
                    <div className="text-[#B5482F] text-[11.5px] mt-1.5">
                      {imageError}
                    </div>
                  )}
                </div>
            ...
            <Field label="Supplier name">
              <TextInput
                ...
                placeholder="e.g. ABC Fabric Supply"
              />
            </Field>
            ...
            <Field
              label={`Stock on hand (${ITEM_UNITS.find((u) => u.value === form.unit)?.label || form.unit})`}
            >
              <TextInput
                type="number"
                value={form.stock}
            ...
            <div
              className={`flex justify-between text-[12.5px] font-mono mb-3.5 -mt-1.5 px-2.5 py-1.5 bg-[#F1EDE3] rounded ${formMargin < 0 ? "text-[#B5482F]" : "text-[#3D6B5C]"}`}
            >
            ...
            <div className="flex gap-2">
              <button
                onClick={addOrUpdate}
                className="flex-1 bg-[#1B2A3D] text-[#FAF8F3] border-none px-3 py-2 rounded-md text-[13.5px] font-semibold"
              >
              ...
              {editingId && (
                <button
                  ...
                  className="bg-transparent border border-[#DAD5C6] px-3 py-2 rounded-md text-[13.5px]"
                >
                  Cancel
                </button>
              )}
            ...
            <EmptyState
              ...
              body={
                sq
                  ...
                  : "Register products or services here so you can add them to invoices with one click."
              }
            />
          ) : (
                  ...
                  <thead>
                    <tr className="bg-[#F1EDE3] text-left">
                      {[
                        ...
                        <th
                          key={i}
                          className="font-mono text-[10.5px] tracking-[0.06em] uppercase text-[#8A8574] font-semibold px-3.5 py-2.5"
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {filtered.map((it, idx) => {
                      ...
                      return (
                        <tr
                          key={it.id}
                          className={`border-t border-[#E4DFD3] ${idx % 2 ? "bg-[#FCFAF5]" : "bg-[#FFFDF9]"}`}
                        >
                          ...
                          <td className="font-mono px-3.5 py-2.5">
                            {it.code || "—"}
                          </td>
                          <td className="px-3.5 py-2.5">{it.desc}</td>
                          <td className="text-[#6E6A5C] px-3.5 py-2.5">
                            {it.supplierName || "—"}
                          </td>
                          <td className="text-[#6E6A5C] px-3.5 py-2.5">
                            {ITEM_UNITS.find((u) => u.value === it.unit)
                              ?.label || it.unit}
                          </td>
                          <td
                            className={`font-mono font-semibold px-3.5 py-2.5 ${num(it.stock) <= 0 ? "text-[#B5482F]" : "text-[#1B2A3D]"}`}
                          >
                            {num(it.stock)}
                          </td>
                          <td className="font-mono text-[#6E6A5C] px-3.5 py-2.5">
                            {money(num(it.cost))}
                          </td>
                          <td className="font-mono px-3.5 py-2.5">
                            {money(num(it.rate))}
                          ...
                          <td
                            className={`font-mono font-semibold px-3.5 py-2.5 ${m < 0 ? "text-[#B5482F]" : "text-[#3D6B5C]"}`}
                          >
                            {m.toFixed(1)}%
                          </td>
                          <td className="text-right px-3.5 py-2.5">
                            <div className="flex gap-1.5 justify-end">
                              <button
                                onClick={() => edit(it)}
                                className="bg-transparent border border-[#DAD5C6] rounded-md px-2 py-1 text-xs"
                              >
                                Edit
                              </button>
                              <button
                                onClick={() => {
                                  if (confirm(`Remove ${it.desc}?`))
                                    remove(it.id);
                                }}
